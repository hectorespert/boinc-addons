---
mode: agent
description: Build, run, and smoke-test the boinc and boinctui Home Assistant add-ons via Docker.
---

Build, run, and verify the `boinc` and `boinctui` Home Assistant add-ons using
the driver already committed in this repo — don't reinvent these steps or
...
`boinctui`); each subcommand cleans up its own containers/volumes on success.
